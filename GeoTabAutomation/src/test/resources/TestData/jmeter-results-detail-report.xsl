...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="titleReport" select="'Automation Script Test Result'" />
	...
	<xsl:template name="summary">
		...
		<table align="center" class="details" border="0" cellpadding="5"
			...
			<tr valign="top">
				...
				<td align="center" id="FailureCount">
	...
	<xsl:template name="detail">
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<table align="center" class="details" border="0" cellpadding="5"
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb]//sample[attribute::s='false']">
					<tr>
						<td
 					...
 					<xsl:variable name="message">
						<xsl:for-each select="assertionResult">
							<xsl:choose>
								<xsl:when test="failure= 'true'">
									<xsl:value-of select="concat($lineBreak,failureMessage,$lineBreak)" />
								</xsl:when>
						...
						<xsl:variable name="assertionType">
						<xsl:for-each select="assertionResult">
							<xsl:choose>
								<xsl:when test="failure= 'true'">
									<xsl:value-of select="concat($lineBreak,name,$lineBreak)" /><br/>
								</xsl:when>
								<xsl:when test="error= 'true'">
									<xsl:value-of select="concat($lineBreak,name,$lineBreak)" /><br/>
								</xsl:when>
						    ...
						    <td
								style="white-space:pre-wrap;white-space:-moz-pre-wrap;white-space:-pre-wrap;white-space:-o-pre-wrap;word-wrap:break-word;font-weight:bold; color:red;">
								<xsl:value-of select="$assertionType" />
							</td>
						<td
								style="white-space:pre-wrap;white-space:-moz-pre-wrap;white-space:-pre-wrap;white-space:-o-pre-wrap;word-wrap:break-word;font-weight:bold; color:red;">
